...
// Le select Supabase (sans types générés) infère `profiles` comme un tableau,
// alors que PostgREST renvoie un objet (relation many-to-one) et `companies`
// comme un tableau (one-to-many). On déclare la forme réelle, comme FeedPost.
type Participant = {
  profile_id: string;
  profiles: {
      ...
      <div className="flex flex-col divide-y divide-line bg-white border border-line rounded-lg overflow-hidden">
        {conversations?.map((conv) => {
          const otherParticipant = (
            conv.conversation_participants as unknown as Participant[] | undefined
